...
- 대신 이 앱은 브라우저에 남아있는 구글 로그인 세션을 이용해 **조용히(팝업 없이) 자동으로
  액세스 토큰을 재발급**받습니다 — 페이지를 다시 열 때, 그리고 토큰이 만료되기 5분 전에
  미리 한 번씩. 구글 계정에 로그인된 상태가 유지되는 한, 체감상 "로그인이 유지되는" 것처럼
  동작합니다.
- 다음의 경우에는 다시 "Google로 로그인" 버튼을 눌러야 합니다: 브라우저에서 구글 계정을
  ...
  직접 취소한 경우, 브라우저 데이터(쿠키)를 지운 경우, 또는 OAuth 동의 화면이 "테스트"
  상태라서 애초에 세션이 오래 유지되지 않는 경우.
