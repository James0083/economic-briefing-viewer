...
3. **API 및 서비스 > OAuth 동의 화면**을 구성합니다.
   ...
   - "테스트" 상태로 두면 **테스트 사용자**로 등록한 계정만 로그인할 수 있고, 등록하지
     않은 계정은 `403: access_denied`로 막힙니다. 동의도 7일마다 만료됩니다.
     **"프로덕션으로 게시"** 하면 두 제약이 모두 사라집니다(심사 신청까지는 필요 없음).
     이 저장소의 앱은 현재 **프로덕션** 상태입니다.
...
- 동의 화면을 **프로덕션**으로 게시했으므로 테스트 사용자 등록도, 7일 동의 만료도
  적용되지 않습니다. 다만 검증(verification)을 받지 않은 상태라 "확인하지 않은 앱" 경고는
  계속 뜨고, **동의를 허용한 사용자 수가 프로젝트 전체 기간 통틀어 100명으로 제한**됩니다
  (개인용으로는 사실상 무제한).
- 완전 자동 재로그인이 꼭 필요하다면 Google 앱 검증을 받아야 하는데, `drive.readonly`는
  구글이 **제한된 범위(restricted scope)** 로 분류하는 Drive 범위라 검증에 제3자 **CASA
  보안 평가**까지 요구됩니다. 개인용 소규모 앱에는 현실적이지 않으므로 지금의 "클릭 한 번"
  방식을 유지합니다.

## 새 사용자(뷰어) 추가하기

동의 화면이 **프로덕션** 상태이므로 **Google Drive 공유만 하면 됩니다.**

- "경제시황분석 - Claude" 폴더 우클릭 > 공유 > 이메일 입력, 권한은 **뷰어(읽기 전용)**.

새 사용자는 첫 로그인 때 "Google에서 확인하지 않은 앱" 경고를 만납니다.
**고급 > "…(안전하지 않음)으로 이동"** 을 눌러야 넘어가므로 미리 알려주세요.

> 동의 화면이 "테스트" 상태라면 여기에 더해 Cloud Console > OAuth 동의 화면 >
> 테스트 사용자 > **+ ADD USERS** 등록이 필요합니다. 등록하지 않은 계정은 Drive 공유가
> 돼 있어도 `403: access_denied`로 로그인 자체가 막힙니다.
