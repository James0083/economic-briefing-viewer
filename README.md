...
- **모바일 반응형**: 좁은 화면에서는 파일 목록이 좌측 슬라이드 메뉴(헤더의 ☰ 버튼)로 표시됩니다.
  본문 영역 어디에서든 가로로 스와이프해서 여닫을 수도 있습니다(오른쪽으로 밀면 열림,
  열린 상태에서 왼쪽으로 밀면 닫힘). 화면 좌우 가장자리는 안드로이드 시스템 "뒤로 가기"
  제스처 영역이라 그 부근에서 시작한 스와이프는 인식하지 않습니다.
- **계정 메뉴**: 헤더 오른쪽의 동그란 버튼(이메일 첫 글자)을 누르면 로그인한 계정과
  로그아웃 버튼이 나옵니다. 로그아웃하면 저장된 토큰이 지워지고 로그인 전 화면으로
  돌아갑니다(구글 계정 자체에서 로그아웃되지는 않습니다).
...
3. **API 및 서비스 > OAuth 동의 화면**을 구성합니다.
   ...
   - **"테스트" 상태를 유지하세요.** `drive.readonly`는 구글이 **제한된 범위
     (restricted scope)**로 분류하는데, 이런 범위는 검증(verification)을 받지 않은 앱이
     "프로덕션"으로 게시되면 통과 버튼조차 없이 **로그인 자체가 완전히 차단**됩니다
     ("...이(가) Google의 인증절차를 완료하지 않았습니다" 화면). "테스트" 상태에서는
     등록된 **테스트 사용자**(최대 100명)에 한해 "확인하지 않은 앱" 경고 후 **고급 > 이동**
     버튼으로 정상 로그인할 수 있습니다. 대신 테스트 사용자로 등록하지 않은 계정은
     `403: access_denied`로 막히고, 동의가 7일마다 만료됩니다(재로그인으로 갱신).
...
- **한 번 로그인하면 토큰 유효기간(약 1시간) 동안은 새로고침해도 로그인이 유지됩니다.**
  발급받은 액세스 토큰을 만료 시각과 함께 `localStorage`에 저장해두고, 페이지를 다시 열 때
  아직 유효하면 구글에 다녀오지 않고 바로 이어서 씁니다. 토큰이 만료되면 로그인 버튼을 다시
  눌러야 합니다.
- **토큰이 만료된 뒤의 재로그인은 버튼 클릭이 필요합니다.** 완전히 백엔드가 없는 정적
  사이트는 Google로부터 refresh token을 발급받을 수 없고, `drive.readonly`처럼 민감한
  범위를 요청하는 미검증 앱은 로그인 시 "Google에서 확인하지 않은 앱" 경고가 항상 뜨는데
  이 화면은 사용자가 직접 클릭해야만 넘어갈 수 있어서, 화면 없이 조용히 재발급받는
  방식(`prompt=none`)으로는 우회할 수 없기 때문입니다. 대신 이미 구글 계정에 로그인·동의한
  상태라면 비밀번호 재입력 없이 클릭 한두 번으로 빠르게 들어갈 수 있습니다.
- 동의 화면은 **"테스트" 상태를 유지해야 합니다.** `drive.readonly`가 제한된 범위라
  검증 없이 "프로덕션"으로 게시하면 로그인이 완전히 막힙니다. "테스트" 상태에서는
  테스트 사용자 등록 + 7일 동의 만료를 감수하는 대신 정상적으로 로그인할 수 있습니다.
- 완전 자동 재로그인 + 프로덕션 게시가 꼭 필요하다면 Google 앱 검증을 받아야 하는데,
  제한된 범위(restricted scope)는 검증에 제3자 **CASA 보안 평가**까지 요구됩니다.
  개인용 소규모 앱에는 현실적이지 않으므로 지금의 "테스트 상태 + 클릭 한 번" 방식을
  유지합니다.
...
1. **Google Cloud Console (로그인 허용)** — Cloud Console > OAuth 동의 화면 >
   테스트 사용자 > **+ ADD USERS** > 이메일 입력. (동의 화면을 "프로덕션"으로 게시하면
   안 됩니다 — `drive.readonly`가 제한된 범위라 검증 없이는 로그인이 완전히 막힙니다.)
2. **Google Drive (파일 접근 권한)** — "경제시황분석 - Claude" 폴더 우클릭 > 공유 > 이메일
   입력, 권한은 **뷰어(읽기 전용)**.

새 사용자는 첫 로그인 때 "Google에서 확인하지 않은 앱" 경고를 만납니다.
**고급 > "…(안전하지 않음)으로 이동"** 을 눌러야 넘어가므로 미리 알려주세요.
...
- OAuth 동의 화면은 "테스트" 상태를 유지해야 하며, 등록된 테스트 사용자만 로그인할 수 있고
  동의가 7일 후 만료됩니다.
